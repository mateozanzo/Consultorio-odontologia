...
def ver_agenda_diaria_medico():
    ...
    if not nombres_turnos:
        return "No hay turnos registrados."
    id_ = next((i for i, n in zip(id_doctor, nombres_doctor) if usuario_actual.split("@")[0].lower() in n.lower()), None)
    if id_ is None:
    ...
    while True:
        ...
        try:
            ...
            if horario < datetime.now():
                print("No se puede seleccionar una fecha y hora pasada.")
                continue
...
def imprimir_recibo(usuario, perfil):
    ...
    try:
        id = int(input("Ingrese el número de socio (paciente): ")) if perfil == "paciente" else next((i for i, n in zip(id_doctor, nombres_doctor) if usuario.split("@")[0].lower() in n.lower()), None)
    except ValueError:
        return "ID inválido."
